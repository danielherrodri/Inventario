...
export const removelistenersInputs = (selector) => {
    ...
    const alertas = ["border-l-4", "border", "border-red-500"];
...
function validarInput(e) {
    ...
    if (clases.includes("border-red-500")) {
        if (e.target.nextElementSibling === null) {
            ...
            errorDiv.classList.add("block", "text-red-500", "text-xs", "mt-2");
            // insertar el error
            ...
            // clase al label
            e.target.previousElementSibling.classList.add("text-red-500");
        }
    } else {
        // limpiar el mensaje de error si existe
        if (
            e.target.nextElementSibling !== null &&
            !e.target.nextElementSibling.classList.contains("no-blur")
        ) {
            e.target.nextElementSibling.remove();
            e.target.previousElementSibling.classList.remove("text-red-500");
...
export const validateForm = (array) => {
    // Clases
    const alert = ["block", "text-red-500", "text-xs", "mt-2"];
    const noValido = ["border-l-4", "border-red-500"];
    ...
    for (let elem of array) {
        ...
        if (
            elem.classList.contains("required") ||
            elem.classList.contains("radio")
        ) {
            ...
            if (elem.nextElementSibling)
                elem.nextElementSibling.classList.remove(...noValido);
            if (elem.previousElementSibling)
                elem.previousElementSibling.classList.remove("text-red-500");

            if (elem.nodeName == "INPUT" && elem.type !== "file") {
                if (elem.value == "") {
                    if (document.querySelector(`[input="${elem.name}"]`)) {
                        const trix = document.querySelector(
                            `[input="${elem.name}"]`
                        );
                    ...
                    elem.parentElement.appendChild(div);
                    elem.classList.add(...noValido);
                    elem.previousElementSibling.classList.add("text-red-500");
                    elem.focus();
                    return false;
                ...
                // Obtener radios
                const radios = [
                    ...document.querySelectorAll(`[name="${elem.name}"]`),
                ];
                ...
                msj_warning(
                    "Para el archivo comprimido debes seleccionar que contiene de las opciones mostradas"
                );
                return false;
            }
        }
...
export const runReplacers = () => {
    ...
    if (rscc.length > 0) {
        rscc.forEach((element) => {
            element.addEventListener("input", (e) => {
                let data = replaceSpecialCharactersAndCharacters(
                    e.target.value
                );
                e.target.value = data;
                e.target.focus();
...
// Clase rscn
export const replaceSpecialCharactersAndNumbers = (string) =>
    string.replace(
        /\$|\!|\¡|\"|\#|\$|\%|\&|\/|\(|\)|\=|\¿|\?|\\|\{|\}|\[|\]|\;|\:|\-|\_|\^|\*|\'|\<|\>|\||\°|\¬|\@|[0-9]+/g,
        ""
    );
...
// Clase rscc
export const replaceSpecialCharactersAndCharacters = (string) =>
    string.replace(/\D/g, "");

// Clase ond
export const onlyDecimalNumbers = (string) =>
    string.replace(
        /\$|\!|\¡|\"|\#|\$|\%|\&|\/|\(|\)|\=|\¿|\?|\\|\{|\}|\[|\]|\;|\:|\-|\_|\^|\*|\'|\<|\>|\||\°|\¬|\@|\+|[A-z]+/g,
        ""
    );
...
// Clase rscorder
export const replaceSpecialCharactersOrder = (string) =>
    string.replace(
        /\$|\!|\¡|\"|\#|\$|\%|\&|\/|\(|\)|\=|\¿|\?|\\|\{|\}|\[|\]|\;|\:|\_|\^|\*|\'|\<|\>|\||\°|\¬|\@|[A-z]+/g,
        ""
    );
...
export const input_money = (element) => {
    ...
    input.addEventListener("input", (element) => {
        let datos = element.target.value
            .replace(/\D/g, "")
...
export const ShowMessageLoading = () => {
    ...
    load.append(
        "<h5 class='text-loader' style='width: 200px; text-align: center'>Guardando datos por favor espera...</h5>"
    );
    load.css("visibility", "");
    load.css("opacity", "");
...
export const showSpinner = () => {
    ...
    spinnerLoader.classList.toggle("hidden");
};
...
export const spinnerText = `<div class="sk-chase">
                                <div class="sk-chase-dot"></div>
                                <div class="sk-chase-dot"></div>
                                <div class="sk-chase-dot"></div>
                                <div class="sk-chase-dot"></div>
